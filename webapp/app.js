...
function defaultProfile(name='Default'){
  ...
  return {
    ...
    schedule:{term_start_date:'',course_meetings:{},imported_occurrences:[],display_options:{show_ampm:true,show_class_title:false,show_instructors:false,days:Object.fromEntries(DAY_NAMES.map(d=>[d,true])),start_minute:480,end_minute:1350},calendar_week:dateKey(mondayOf(new Date())),personal_events:[]},
...
function normalizeState(raw){
  ...
  for(const p of Object.values(out.profiles)){
    ...
    p.schedule ||= defaultProfile().schedule; p.schedule.course_meetings ||= {}; p.schedule.personal_events ||= []; p.schedule.display_options ||= defaultProfile().schedule.display_options; p.schedule.calendar_week ||= dateKey(mondayOf(new Date()));
...
function renderSidebar(){
  ...
  const primary=[['dashboard','⌂','Dashboard'],['notes','✎','Notes'],['schedule','▦','Schedule'],['settings','⚙','Settings']];
...
function renderCourse(){
  ...
  c.ui_state ||= {last_tab:'Overview'}; if(!route.tab) route.tab=c.ui_state.last_tab||'Overview'; c.ui_state.last_tab=route.tab;
  setHeader(c.code, c.name||'Course workspace');
  const tabs=['Overview','Study Material','Questions','Testing'];
  let body=''; if(route.tab==='Overview') body=courseOverview(c); else if(route.tab==='Study Material') body=courseMaterials(c); else if(route.tab==='Questions') body=courseQuestions(c); else body=courseTesting(c);
...
function courseOverview(c){
  ...
  return `<section class="card"><div class="card-header"><div><h2>Events</h2><div class="card-subtitle">Homework, exams and other deadlines for ${escapeHtml(c.code)}</div></div><button id="add-event" class="primary-button">＋ Event</button></div><div class="list">${events.length?events.map(e=>`<div class="list-item"><div class="list-main"><div class="list-title">${escapeHtml(e.title||e.type||'Event')}</div><div class="list-copy">${formatDateTime(e.due_at)}${e.note?` • ${escapeHtml(e.note)}`:''}</div></div><span class="pill">${escapeHtml(e.type||'Event')}</span><button class="danger-button" data-delete-event="${e.id}">Delete</button></div>`).join(''):'<div class="empty">No events saved.</div>'}</div></section>`;
}
...
function bindCoursePage(c){
  bindPageCommon();
  els.page.querySelectorAll('[data-course-tab]').forEach(b=>b.onclick=()=>{route.tab=b.dataset.courseTab;c.ui_state.last_tab=route.tab;queueSave();renderCourse();});
...
async function createLectureDialog(c){
  ...
  modal(`<h2>New Lecture</h2><div class="form-grid"><div class="field full"><label>Title</label><input id="lecture-title" value="Untitled Document"></div><div class="field full"><label>Unit</label><select id="lecture-unit"><option value="">Ungrouped</option>${unitOptions}</select></div></div>`, async root=>{
...
function scheduleOccurrences(week){ const p=activeProfile(),s=p.schedule,out=[]; const weekStart=mondayOf(week); for(const c of orderedCourses(p)) for(const m of s.course_meetings[c.id]||[]){ if(m.enabled===false)continue; const di=DAY_NAMES.indexOf(m.day);if(di<0)continue; const date=addDays(weekStart,di); if(m.recurrence==='Once'&&m.first_class_date!==dateKey(date))continue; if(m.recurrence==='Bi-weekly'&&m.first_class_date){const anchor=mondayOf(parseDateKey(m.first_class_date));const diff=Math.round((weekStart-anchor)/604800000);if(diff%2!==0)continue;} out.push({...m,course:c,date}); }
...
function bindScheduleCalendar(){ const s=activeProfile().schedule;document.getElementById('week-prev').onclick=()=>{s.calendar_week=dateKey(addDays(parseDateKey(s.calendar_week),-7));queueSave();renderSchedule();};document.getElementById('week-next').onclick=()=>{s.calendar_week=dateKey(addDays(parseDateKey(s.calendar_week),7));queueSave();renderSchedule();};document.getElementById('week-today').onclick=()=>{s.calendar_week=dateKey(mondayOf(new Date()));queueSave();renderSchedule();};drawCalendarEvents(); liveTimer=setInterval(drawCalendarLiveLine,60000);drawCalendarLiveLine(); }
