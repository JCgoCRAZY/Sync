...
(() => {
  ...
  function installInfoEditing() {
    ...
    if (!root || !popover || !content || popover.dataset.editUpgradeInstalled === "true") return;
    popover.dataset.editUpgradeInstalled = "true";

    // The canonical editor already has the complete +Info edit/save machinery.
    // Existing Info highlights intentionally open read-only; clicking the text in
    // that open popover now re-opens the same Info in the editor's native editing
    // mode. This keeps the exact original save/close/autosave path instead of
    // duplicating Quill formatting logic in this enhancement layer.
    const style = document.createElement("style");
    style.textContent = `
      #info-popover:not([hidden]) #info-popover-content:not([contenteditable="true"]):hover{
        background:rgba(255,255,255,.055);
        box-shadow:inset 0 0 0 1px rgba(38,200,184,.45);
        border-radius:5px;
      }
    `;
    document.head.appendChild(style);
    ...
    content.addEventListener("click", event => {
      if (popover.hidden || content.isContentEditable) return;
      if (typeof openInfoPopover !== "function" || typeof activeInfoId === "undefined" || !activeInfoId) return;
      event.preventDefault();
      event.stopPropagation();
      openInfoPopover(activeInfoId, true);
    });
  }
  ...
  function installStableDividerResize() {
    ...
    function end(event) {
      ...
      requestAnimationFrame(() => {
        ...
        // Do NOT synthesize a window resize here. The canonical editor's resize
        // listener releases and re-renders every visible PDF canvas. Triggering
        // that while the split pane has just changed size can overlap PDF.js canvas
        // work and visually invert/corrupt a page. Existing canvases scale safely
        // with their page frames; a real browser resize can refresh resolution later.
      });
    }
