(() => {
  ...
  function updateMainState(mutator){
    const task=stateSerial.then(async()=>{
      ...
      if(cloud()?.markStateDirty) await cloud().markStateDirty();
      return s;
    });
  ...
  window.addEventListener('studyhub-cloud-status',event=>{
    ...
    if(s.phase==='conflict'){statusEl.textContent='Saved on this device • cloud conflict — return to Settings to resolve';statusEl.className='error';}
    else if(s.phase==='offline'){statusEl.textContent=s.pendingChanges?`Saved on this device • offline • ${s.pendingChanges} waiting`:'Saved on this device • offline';}
    else if(s.phase==='syncing'||s.phase==='pending'){statusEl.textContent=s.pendingChanges?`Saved on this device • cloud syncing (${s.pendingChanges})…`:'Saved on this device • cloud syncing…';}
  });
