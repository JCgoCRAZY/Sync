(() => {
  ...
  async function saveDocumentObject(doc){doc.updated_at=nowIso();await put(STORES.documents,materialId,doc);cloud()?.markDocumentDirty?.(materialId).catch(console.error);return doc;}
  async function updateMainState(mutator){const s=await get(STORES.meta,'state');if(!s)return;mutator(s);s.app ||= {};s.app.last_saved=nowIso();await put(STORES.meta,'state',s);cloud()?.markStateDirty?.().catch(console.error);}
  ...
  const api = {
    ...
    async request_close(){try{await updateMaterialMeta((await loadDocument()).title);await cloud()?.flushNow?.();}catch(_e){} if(history.length>1)history.back();else location.href='../index.html';return {ok:true};}
  };
  ...
  async function assignSelectedPdf(){
    ...
    try{if(!file.name.toLowerCase().endsWith('.pdf'))throw new Error('Slides must be a PDF.');await put(STORES.blobs,`pdf:${materialId}`,file);cloud()?.markBlobDirty?.(`pdf:${materialId}`).catch(console.error);const d=await loadDocument();const previousCloudObject=d.slides_attachment?.cloud_object||d.slides_attachment?.previous_cloud_object||null;d.slides_attachment={relative_path:`browser-pdf:${materialId}`,display_name:file.name,size_bytes:file.size,assigned_at:nowIso(),previous_cloud_object:previousCloudObject};d.editor_layout={...(d.editor_layout||{}),slides_visible:true};await saveDocumentObject(d);const config=await makePdfConfig();window.editorApp?.setPdfServiceConfig(config);setTimeout(()=>frame?.dispatchEvent(new Event('load')),20);}catch(err){console.error(err);const status=document.getElementById('save-status');if(status){status.textContent=`Slides import failed: ${err.message||err}`;status.className='error';}setTimeout(()=>frame?.dispatchEvent(new Event('load')),20);}
  }
  ...
  window.addEventListener('studyhub-cloud-status',event=>{
    ...
    if(s.phase==='conflict'){statusEl.textContent='Saved locally • cloud conflict — return to Settings to resolve';statusEl.className='error';}
    else if(s.phase==='offline'){statusEl.textContent=s.pendingChanges?`Saved locally • offline • ${s.pendingChanges} waiting`:'Saved locally • offline';}
    else if(s.phase==='syncing'||s.phase==='pending'){statusEl.textContent=s.pendingChanges?`Saved locally • cloud syncing (${s.pendingChanges})…`:'Saved locally • cloud syncing…';}
  });
