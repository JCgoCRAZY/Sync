...
(() => {
  ...
  function openScheduleCalendarFromShortcut() {
    ...
    const isCurrentCalendarView =
      route.page === "schedule" &&
      (profile.schedule._web_view || "calendar") === "calendar";

    if (isCurrentCalendarView) {
  ...
  function indexOfCurrent(list) {
    ...
    if (route.page === "all_courses_unit") {
      return list.findIndex(item => item.page === "all_courses");
    }
  ...
  document.addEventListener("keydown", event => {
    ...
    // Main-app numeric course shortcuts: 1-9 open the course in that ranked
    // sidebar position. The current route tab is preserved as the most recently
    // used course tab (Overview or Study Material).
    if (/^[1-9]$/.test(event.key) && !event.repeat && !blocked(event) && scheduleShortcutAllowed()) {
      const rank = Number(event.key) - 1;
      const course = orderedCourses()[rank];
      if (course) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        ...
        try {
          ...
          navTo("course", {
            courseId: course.id,
            tab: typeof normalizeCourseTab === "function"
              ? normalizeCourseTab(route?.tab)
              : (route?.tab === "Study Material" ? "Study Material" : "Overview")
          });
        } catch (error) {
          console.error("Course number shortcut failed", error);
          if (typeof toast === "function") {
            toast("Course shortcut could not open. Try again.", 3000);
          }
        }
      }
      return;
    ...
    if (!["dashboard","notes","course","all_courses","all_courses_unit","schedule","settings"].includes(route.page)) return;
