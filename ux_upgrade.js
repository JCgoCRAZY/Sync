...
(() => {
  ...
  function updateMostRecentSidebar() {
    ...
    const signature = recent
      ? `${recent.course.id}|${recent.material.id}|${recent.material.title || ''}|${recent.material.updated_at || ''}|${recent.material.last_opened_at || ''}`
      : 'empty';

    // Avoid rebuilding this sidebar card on every route render when nothing changed.
