...
(() => {
  ...
  function findMostRecentMaterial() {
    ...
    for (const course of orderedCourses()) {
      for (const material of course.materials || []) {
        ...
        if (!best || ts > best.ts) best = { course, material, ts };
      }
    }
    return best;
  }
  ...
  function updateMostRecentSidebar() {
    ...
    section.innerHTML = `<h3>Most recent</h3>
      <div class="most-recent-card">
        <div class="material-icon">📄</div>
        <div class="most-recent-copy">
          <strong>${escapeHtml(recent.material.title || 'Untitled Document')}</strong>
          <small>Updated ${escapeHtml(formatCompactDate(recent.material.updated_at || recent.material.last_opened_at))}</small>
        </div>
        <button class="primary-button" id="most-recent-open">Open</button>
      </div>`;
    section.querySelector('#most-recent-open')?.addEventListener('click', event => {
      event.stopPropagation();
      openMaterial(recent.course.id, recent.material.id);
    });
  }
  ...
  function installStudyMaterialSearch(course) {
    ...
    input.addEventListener('input', () => {
      ...
      timer = setTimeout(() => searchCourseDocuments(course, input.value, results, token), 120);
    });
  ...
  renderCourse = function uxRenderCourse() {
    ...
    if (c && route.tab === 'Study Material') installStudyMaterialSearch(c);
  ...
  function todaysClassesHtml(items) {
    ...
    return `<div class="today-classes-row">${items.map(item => {
      ...
      return `<button class="today-class" data-course-open="${escapeHtml(item.course.id)}">
  ...
  function installReadyHooks() {
    ...
    deviceHeartbeatTimer = setInterval(() => registerDevicePresence(false), 15 * 60 * 1000);
    ...
    if (recent) {
      ...
      idle(() => idbGet(STORES.documents, recent.material.id).catch(() => {}));
    }
