...
(() => {
  ...
  function allCoursesUnitRowHtml(course, row) {
    ...
    const lectureButtons = materials.length
      ? materials.map(material => `<button class="all-courses-lecture-link" type="button"
          data-all-open-lecture="${escapeHtml(course.id)}|${escapeHtml(material.id)}"
          title="Open ${escapeHtml(material.title || 'Untitled Document')}">${escapeHtml(material.title || 'Untitled Document')}</button>`).join('')
      : '<span class="all-courses-no-lectures">No lectures</span>';

    return `<div class="all-courses-unit-row">
      <div class="all-courses-unit-name">${escapeHtml(row.title)}</div>
      <div class="all-courses-unit-lectures">${lectureButtons}</div>
  ...
  function bindAllCoursesPage() {
    ...
    els.page?.querySelectorAll('[data-all-open-lecture]').forEach(button => {
      button.addEventListener('click', () => {
        const [courseId, materialId] = String(button.dataset.allOpenLecture || '').split('|');
        if (courseId && materialId) openMaterial(courseId, materialId);
      });
    });
