...
const APP_VERSION = '2026.09.09-sync-hardening-7';
...
let stateSaveSerial = Promise.resolve();
let toastTimer = null;
let liveTimer = null;
let dbPromise = null;
let audioTrack = null;
...
function cloudPhaseLabel(s=cloudStatus()){
  ...
  if(s.phase==='error'||s.phase==='degraded')return s.pendingChanges?`⚠ Cloud (${s.pendingChanges})`:'⚠ Cloud';
  return '☁ Cloud';
}
...
function syncTimeLabel(value){return value?formatDateTime(value):'Not yet';}
function cloudSettingsHtml(){
  const s=cloudStatus(),user=cloudApi()?.getUser?.();
  if(!s.available)return `<section class="card"><div class="card-header"><div><h2>Cloud Sync</h2><div class="card-subtitle">Supabase is unavailable. The app is still saving locally and will reconnect automatically.</div></div></div></section>`;
  ...
  return `<section class="card"><div class="card-header"><div><h2>Cloud Sync & Save Reliability</h2><div class="card-subtitle">Local-first saves • exact cloud acknowledgements • persistent retry queue • granular course/Lecture sync • recovery history</div></div></div>
    <div class="cloud-status-grid"><div><div class="cloud-status-line"><span class="cloud-status-dot ${escapeHtml(s.phase||'')}"></span><strong>${escapeHtml(s.message||'Cloud')}</strong></div><div class="cloud-account-email">${escapeHtml(user?.email||'')} • Device …${escapeHtml(shortDevice)}</div></div></div>
    <div class="sync-save-state-grid">
      <div class="sync-save-state"><span>1</span><div><strong>Saved locally</strong><small>${escapeHtml(syncTimeLabel(s.lastLocalSaveAt))}</small></div></div>
      <div class="sync-save-state"><span>2</span><div><strong>${pending?'Awaiting cloud':'Cloud confirmed'}</strong><small>${pending?`${pending} pending change${pending===1?'':'s'}`:escapeHtml(syncTimeLabel(s.lastCloudConfirmedAt||s.lastSync))}</small></div></div>
      <div class="sync-save-state"><span>3</span><div><strong>Automatic recovery</strong><small>10 local + 10 cloud Lecture versions</small></div></div>
    </div>
    ${pending?`<div class="cloud-warning"><strong>${pending}</strong> local change${pending===1?'':'s'} safely waiting for cloud confirmation.${s.retryAt?` Next retry: ${escapeHtml(syncTimeLabel(s.retryAt))}.`:''}</div>`:''}
    ${s.lastError?`<div class="cloud-warning">Last cloud issue: ${escapeHtml(s.lastError)}</div>`:''}
    ...
    <div class="action-row cloud-tools"><button id="cloud-sync-now" class="primary-button">Retry / Sync now</button><button id="cloud-verify" class="secondary-button">Verify cloud copy</button><button id="cloud-recovery" class="secondary-button">Recovery Center</button><button id="cloud-signout" class="secondary-button">Sign out</button></div>
    <div id="cloud-diagnostics" class="sync-diagnostics"><div class="form-help">Loading sync diagnostics…</div></div>
    <details class="cloud-advanced"><summary>Advanced / migration</summary><div class="cloud-warning" style="margin-top:10px"><strong>Cloud-master migration:</strong> use only on the one device containing the complete workspace you want everywhere. <button id="cloud-make-master" class="secondary-button" style="margin-left:8px">Make this device cloud master</button></div></details>
    <p class="form-help">Normal changes sync only the affected profile/course/schedule/notes item or Lecture. A full workspace upload is kept only as a periodic recovery checkpoint.</p>
  </section>`;
}
async function loadCloudDiagnostics(){
  const box=document.getElementById('cloud-diagnostics'),api=cloudApi();if(!box||!api?.getSyncDiagnostics)return;
  try{
    const d=await api.getSyncDiagnostics();
    const retry=d.retry?.nextRetryAt?syncTimeLabel(d.retry.nextRetryAt):'None';
    const recent=(d.recentSaves||[]).slice(0,5);
    box.innerHTML=`<div class="sync-diag-grid"><div><span>Local save</span><strong>${escapeHtml(syncTimeLabel(d.lastLocalSaveAt))}</strong></div><div><span>Cloud confirmation</span><strong>${escapeHtml(syncTimeLabel(d.lastCloudConfirmedAt||d.lastSync))}</strong></div><div><span>Pending</span><strong>${Number(d.pendingChanges||0)}</strong></div><div><span>Retry scheduled</span><strong>${escapeHtml(retry)}</strong></div><div><span>State items</span><strong>${Number(d.localEntityCount||0)}</strong></div><div><span>Lectures</span><strong>${Number(d.localDocumentCount||0)}</strong></div><div><span>Local storage</span><strong>${d.storagePersistent===true?'Persistent':d.storagePersistent===false?'Browser managed':'Unknown'}</strong></div><div><span>Sync engine</span><strong>${escapeHtml(d.syncMode||'—')}</strong></div><div><span>Schema</span><strong>v${Number(d.syncSchemaVersion||0)}</strong></div></div><div class="form-help">Last local save ID: ${escapeHtml(d.lastLocalSaveId||'—')}<br>Last cloud-confirmed ID: ${escapeHtml(d.lastCloudConfirmedSaveId||'—')}</div>${recent.length?`<details class="sync-recent"><summary>Recent save acknowledgements</summary>${recent.map(x=>`<div class="sync-log-row"><strong>${escapeHtml(x.status||'local')}</strong><span>${escapeHtml(x.kind||'save')} • ${escapeHtml(x.targetId||'')}</span><small>${escapeHtml(x.saveId||'')}</small></div>`).join('')}</details>`:''}`;
  }catch(err){box.innerHTML=`<div class="cloud-warning">Diagnostics unavailable: ${escapeHtml(err.message||err)}</div>`;}
}
function recoveryLectureList(){
  const out=[];
  for(const profile of Object.values(state.profiles||{}))for(const course of Object.values(profile.courses||{}))for(const material of course.materials||[])out.push({id:String(material.id),title:material.title||'Untitled Document',course:course.code||'COURSE'});
  return out;
}
function openRecoveryCenter(){
  const items=recoveryLectureList();
  modal(`<h2>Lecture Recovery Center</h2><div class="card-subtitle">Restore one of the last local or cloud-confirmed versions of a Lecture. Restoring creates a new save; it does not destroy the recovery history.</div><div class="list" style="margin-top:14px">${items.length?items.map(x=>`<button class="list-item" data-recovery-lecture="${escapeHtml(x.id)}"><div class="list-main"><div class="list-title">${escapeHtml(x.course)} • ${escapeHtml(x.title)}</div><div class="list-copy">View previous saved versions</div></div></button>`).join(''):'<div class="empty">No Lectures found.</div>'}</div>`,null,{saveLabel:null,wide:true});
  document.querySelectorAll('[data-recovery-lecture]').forEach(btn=>btn.onclick=()=>showLectureRecovery(btn.dataset.recoveryLecture));
}
async function showLectureRecovery(materialId){
  const api=cloudApi();
  const item=recoveryLectureList().find(x=>x.id===String(materialId));
  modal(`<h2>Recovery — ${escapeHtml(item?.course||'')} ${escapeHtml(item?.title||'Lecture')}</h2><div id="recovery-version-list" class="list"><div class="empty">Loading saved versions…</div></div>`,null,{saveLabel:null,wide:true});
  const list=document.getElementById('recovery-version-list');
  try{
    const versions=await api.getDocumentRecoveryHistory(materialId,10);
    list.innerHTML=versions.length?versions.map((v,i)=>`<div class="list-item"><div class="list-main"><div class="list-title">${escapeHtml(v.title||'Untitled Document')}</div><div class="list-copy">${escapeHtml(v.source==='cloud'?'Cloud checkpoint':'Local checkpoint')} • ${escapeHtml(syncTimeLabel(v.savedAt))}${v.revision?` • revision ${v.revision}`:''}</div></div><button class="secondary-button" data-restore-source="${escapeHtml(v.source)}" data-restore-token="${escapeHtml(v.token)}">Restore</button></div>`).join(''):'<div class="empty">No previous versions are available yet. Versions appear automatically as you keep working.</div>';
    list.querySelectorAll('[data-restore-source]').forEach(btn=>btn.onclick=async()=>{if(!confirm('Restore this Lecture version? Your current version will be checkpointed first.'))return;try{toast('Restoring Lecture…',3500);await api.restoreDocumentVersion(materialId,btn.dataset.restoreSource,btn.dataset.restoreToken);closeModal();await reloadStateFromDisk();toast('Lecture restored and queued for cloud confirmation.',4500);}catch(err){toast(`Restore failed: ${err.message||err}`,5000);}});
  }catch(err){list.innerHTML=`<div class="cloud-warning">Could not load recovery history: ${escapeHtml(err.message||err)}</div>`;}
}
function bindCloudSettings(){
  const api=cloudApi();
  document.getElementById('cloud-open-signin')?.addEventListener('click',()=>showAuthGate(true));
  document.getElementById('cloud-sync-now')?.addEventListener('click',async()=>{try{toast('Retrying cloud sync…');await api?.retrySync?.();renderSettings();}catch(err){toast(`Sync failed: ${err.message||err}`,4500);}});
  document.getElementById('cloud-verify')?.addEventListener('click',async()=>{try{toast('Verifying cloud copy…',4000);const result=await api?.verifyCloudCopy?.();if(result?.ok)toast('Verified: this device and cloud match.',5000);else toast(result?.message||'Cloud verification found a mismatch.',6000);await loadCloudDiagnostics();}catch(err){toast(`Verification failed: ${err.message||err}`,5000);}});
  document.getElementById('cloud-recovery')?.addEventListener('click',openRecoveryCenter);
  ...
  document.getElementById('cloud-make-master')?.addEventListener('click',async()=>{
    if(!confirm('Replace the cloud workspace with EVERYTHING currently stored on this device? Use this only on your one authoritative device.'))return;
    ...
    try{toast('Uploading this device as cloud master…',5000);await api?.promoteLocalToCloud?.();renderSettings();toast('Cloud master created.',5000);}catch(err){toast(`Cloud-master upload failed: ${err.message||err}`,6000);}
  });
...
function normalizeState(raw){
  ...
  for(const p of Object.values(out.profiles)){
    ...
    p.schedule ||= defaultProfile().schedule; p.schedule.term_start_date ||= localDateKey(); p.schedule.course_meetings ||= {}; p.schedule.personal_events ||= [];
    const displayDefaults=defaultProfile().schedule.display_options;
    p.schedule.display_options={...displayDefaults,...(p.schedule.display_options||{})};
    p.schedule.display_options.days={...displayDefaults.days,...(p.schedule.display_options.days||{})};
    p.schedule.calendar_week ||= dateKey(mondayOf(new Date()));
...
function queueSave(cloudDirty=true){
  ...
  els.saveIndicator.textContent='Saving locally…'; els.saveIndicator.style.color='var(--muted)';
  // Durable data changes are committed almost immediately. Device-local navigation
  // can remain lightly debounced because it never affects schoolwork content.
  const delay=cloudDirty?40:120;
  clearTimeout(saveTimer); saveTimer=setTimeout(()=>saveStateNow(true,false),delay);
}
async function saveStateNow(show=true,cloudDirty=false){
  ...
  const shouldCloud=Boolean(cloudDirty||pendingCloud);
  const task=stateSaveSerial.then(async()=>{
    await idbSet(STORES.meta,'state',snapshot);
    // Persist the cloud outbox marker before reporting a fully completed state save.
    if(shouldCloud&&cloudApi()?.markStateDirty) await cloudApi().markStateDirty();
  });
  stateSaveSerial=task.catch(err=>{console.error('Main app local save failed:',err);});
  await task;
  if(show){ els.saveIndicator.textContent='✓ Saved locally'; els.saveIndicator.style.color='var(--success)'; }
}
async function flushMainLocalWrites(){
  clearTimeout(saveTimer); saveTimer=null;
  if(stateSavePending||stateCloudDirtyPending) await saveStateNow(false,false);
  await stateSaveSerial.catch(()=>{});
}
...
function handleGlobalNavigationKeydown(event){
  ...
  if(route.page==='schedule'&&scheduleView==='setup'&&event.key==='ArrowRight'){
    event.preventDefault();
    const schedule=activeProfile().schedule;
    schedule._web_view='calendar';
    schedule.calendar_week=dateKey(mondayOf(new Date()));
    queueSave(false);
    renderSchedule();
    return;
  }
  ...
  if(route.page==='course'&&(event.key==='ArrowLeft'||event.key==='ArrowRight')){
    const nextTab=event.key==='ArrowLeft'?'Overview':'Study Material';
    if(normalizeCourseTab(route.tab)!==nextTab){
      event.preventDefault();
      route.tab=nextTab;
      const c=activeCourse();
      if(c){c.ui_state ||= {};c.ui_state.last_tab=nextTab;}
      queueSave(false);
      renderCourse();
    }
    return;
...
function bindCoursePage(c){
  ...
  els.page.querySelectorAll('[data-edit-event]').forEach(b=>b.onclick=()=>{
    const event=c.notifications.find(x=>x.id===b.dataset.editEvent);
    if(event)eventDialog(c,event);
  });
...
async function openMaterial(courseId,materialId){
  ...
  // Opening a Lecture is device-local navigation metadata; it must never
  // create cloud traffic or conflict with the actual Lecture save.
  m.last_opened_at=nowIso(); queueSave(false);
...
function eventDialog(c,e=null){
  const due=e?.due_at?new Date(e.due_at):new Date(Date.now()+86400000);
  const dateValue=`${due.getFullYear()}-${String(due.getMonth()+1).padStart(2,'0')}-${String(due.getDate()).padStart(2,'0')}`;
  const timeValue=`${String(due.getHours()).padStart(2,'0')}:${String(due.getMinutes()).padStart(2,'0')}`;
  modal(`<h2>${e?'Edit':'Add'} Event</h2><div class="form-grid"><div class="field"><label>Type</label><select id="event-type">${['Homework','Midterm 1','Midterm 2','Final','Other'].map(x=>`<option ${e?.type===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>Date</label><input id="event-date" type="date" value="${dateValue}"></div><div class="field"><label>Time</label><input id="event-time" type="time" value="${timeValue}"></div><div class="field full"><label>Title</label><input id="event-title" value="${escapeHtml(e?.title||'')}"></div><div class="field full"><label>Note</label><textarea id="event-note">${escapeHtml(e?.note||'')}</textarea></div></div>`,root=>{
    const date=root.querySelector('#event-date').value;
    const time=root.querySelector('#event-time').value;
    if(!date)return toast('Choose an event date.');
    if(!time)return toast('Choose an event time.');
    const dueAt=new Date(`${date}T${time}`);
    if(Number.isNaN(+dueAt))return toast('Choose a valid event date and time.');
    const data={id:e?.id||uid('event'),type:root.querySelector('#event-type').value,title:root.querySelector('#event-title').value.trim()||root.querySelector('#event-type').value,due_at:dueAt.toISOString(),note:root.querySelector('#event-note').value.trim(),created_at:e?.created_at||nowIso(),updated_at:nowIso()};
    if(e)Object.assign(e,data); else c.notifications.push(data);
    queueSave();closeModal();renderCourse();
  });
}
...
function calendarMinutesLabel(minutes,showAmpm=true){
  const h=Math.floor(minutes/60),m=minutes%60;
  if(!showAmpm)return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
  const suffix=h>=12?'PM':'AM';
  const hour=h%12||12;
  return `${hour}:${String(m).padStart(2,'0')} ${suffix}`;
}
function calendarDisplayOptionsHtml(){
  const s=activeProfile().schedule;
  const o=s.display_options;
  return `<section class="calendar-display-options"><div class="display-options-title">Display Options</div><div class="display-options-body"><div class="display-options-grid"><label class="display-option"><input type="checkbox" data-calendar-option="show_ampm" ${o.show_ampm!==false?'checked':''}><span>Show AM/PM</span></label><label class="display-option"><input type="checkbox" data-calendar-option="show_class_title" ${o.show_class_title?'checked':''}><span>Show Class Title</span></label><label class="display-option"><input type="checkbox" data-calendar-option="show_instructors" ${o.show_instructors?'checked':''}><span>Show Instructors</span></label>${DAY_NAMES.map(day=>`<label class="display-option"><input type="checkbox" data-calendar-day="${day}" ${o.days?.[day]!==false?'checked':''}><span>${day}</span></label>`).join('')}</div><div class="display-options-actions"><button id="calendar-refresh" class="secondary-button">Refresh Calendar</button></div></div></section>`;
}
function scheduleCalendarHtml(){
  const s=activeProfile().schedule;
  const week=parseDateKey(s.calendar_week||dateKey(mondayOf(new Date())));
  const allDays=DAY_NAMES.map((name,index)=>({name,index,date:addDays(week,index)}));
  let visibleDays=allDays.filter(day=>s.display_options.days?.[day.name]!==false);
  if(!visibleDays.length)visibleDays=[allDays[0]];
  const start=s.display_options.start_minute||480,end=s.display_options.end_minute||1350;
  const rows=Math.ceil((end-start)/60);
  const occ=scheduleOccurrences(week);
  const columns=visibleDays.length;
  const minWidth=Math.max(420,64+columns*118);
  return `<div class="schedule-toolbar" style="margin-bottom:12px"><button id="week-prev" class="secondary-button">← Week</button><strong style="flex:1;text-align:center">${allDays[0].date.toLocaleDateString([], {month:'short',day:'numeric'})} – ${allDays[6].date.toLocaleDateString([], {month:'short',day:'numeric',year:'numeric'})}</strong><button id="week-today" class="secondary-button">Today</button><button id="week-next" class="secondary-button">Week →</button></div><div class="week-wrap"><div class="week-grid" style="grid-template-columns:64px repeat(${columns},minmax(118px,1fr));grid-template-rows:46px repeat(${rows},60px);min-width:${minWidth}px"><div class="week-head"></div>${visibleDays.map(x=>`<div class="week-head">${x.name.slice(0,3)}<br><small>${x.date.getDate()}</small></div>`).join('')}${Array.from({length:rows},(_,r)=>{const minute=start+r*60;return `<div class="time-label">${calendarMinutesLabel(minute,s.display_options.show_ampm!==false)}</div>${visibleDays.map(x=>`<div class="day-cell" data-daycell="${x.index}|${minute}"></div>`).join('')}`}).join('')}<div id="schedule-overlay" style="position:absolute;left:64px;right:0;top:46px;bottom:0;display:grid;grid-template-columns:repeat(${columns},1fr);pointer-events:none">${visibleDays.map(x=>`<div style="position:relative;border-left:1px solid transparent" data-overlay-day="${x.index}"></div>`).join('')}</div></div></div><div id="occ-data" hidden>${escapeHtml(JSON.stringify(occ.map(o=>({...o,date:dateKey(o.date),course:o.course?{id:o.course.id,code:o.course.code,settings:o.course.settings}:null}))))}</div>${calendarDisplayOptionsHtml()}`;
}
function bindScheduleCalendar(){
  const s=activeProfile().schedule;
  document.getElementById('week-prev').onclick=()=>changeScheduleCalendarWeek(-1);
  document.getElementById('week-next').onclick=()=>changeScheduleCalendarWeek(1);
  document.getElementById('week-today').onclick=()=>{s.calendar_week=dateKey(mondayOf(new Date()));queueSave(false);renderSchedule();};
  els.page.querySelectorAll('[data-calendar-option]').forEach(input=>input.onchange=()=>{
    s.display_options[input.dataset.calendarOption]=input.checked;
    queueSave();
  });
  els.page.querySelectorAll('[data-calendar-day]').forEach(input=>input.onchange=()=>{
    if(!input.checked){
      const checked=els.page.querySelectorAll('[data-calendar-day]:checked').length;
      if(!checked){input.checked=true;toast('At least one day must remain visible.');return;}
    }
    s.display_options.days ||= {};
    s.display_options.days[input.dataset.calendarDay]=input.checked;
    queueSave();
  });
  document.getElementById('calendar-refresh').onclick=()=>renderSchedule();
  drawCalendarEvents();
  liveTimer=setInterval(drawCalendarLiveLine,60000);
  drawCalendarLiveLine();
}
function drawCalendarEvents(){
  const s=activeProfile().schedule,start=s.display_options.start_minute||480,end=s.display_options.end_minute||1350;
  const week=parseDateKey(s.calendar_week);
  for(const o of scheduleOccurrences(week)){
    const di=Math.round((o.date-mondayOf(week))/86400000);
    const col=els.page.querySelector(`[data-overlay-day="${di}"]`);
    if(!col)continue;
    const top=(o.start_minute-start)/(end-start)*100,height=(o.end_minute-o.start_minute)/(end-start)*100;
    const el=document.createElement('div');
    el.className='schedule-event';
    el.style.top=`${top}%`;
    el.style.height=`${Math.max(height,2)}%`;
    el.style.pointerEvents='auto';
    const bg=o.course?courseColor(o.course):(o.schedule_box_color||'#FFB44A');
    el.style.background=bg;
    el.style.color=o.course?.settings?.schedule_font_color||o.schedule_font_color||'#102010';

    const primary=o.course?(o.course.code||o.title||o.type||'Class'):(o.title||o.type||'Event');
    const detailLines=[];
    if(o.course&&s.display_options.show_class_title){
      const classTitle=(o.course.name||((o.title&&o.title!==o.course.code)?o.title:'')).trim();
      if(classTitle)detailLines.push(classTitle);
    }
    if(o.type&&o.type!==primary&&o.type!=='Personal Event')detailLines.push(o.type);
    if(s.display_options.show_instructors&&o.instructor)detailLines.push(o.instructor);
    const timeText=`${calendarMinutesLabel(o.start_minute,s.display_options.show_ampm!==false)}–${calendarMinutesLabel(o.end_minute,s.display_options.show_ampm!==false)}`;
    const meta=[timeText,o.classroom||''].filter(Boolean).join(' • ');
    el.innerHTML=`<span>${escapeHtml(primary)}</span>${detailLines.map(line=>`<small>${escapeHtml(line)}</small>`).join('')}<small>${escapeHtml(meta)}</small>`;
    if(!o.course)el.onclick=()=>scheduleEventDialog(activeProfile().schedule.personal_events.find(e=>e.id===o.id));
    col.appendChild(el);
  }
}
...
function modal(content,onSave,opts={}){
  const saveLabel=opts.saveLabel===undefined?'Save':opts.saveLabel;
  els.modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal ${opts.wide?'wide':''}">${content}<div class="modal-actions"><button class="secondary-button" data-modal-cancel>${saveLabel? 'Cancel':'Close'}</button>${saveLabel?`<button class="primary-button" data-modal-save>${escapeHtml(saveLabel)}</button>`:''}</div></div></div>`;
  const root=els.modalRoot;
  root.querySelector('[data-modal-cancel]').onclick=closeModal;
  if(saveLabel&&onSave)root.querySelector('[data-modal-save]').onclick=()=>onSave(root);
  root.querySelector('.modal-backdrop').addEventListener('pointerdown',e=>{if(e.target===e.currentTarget)closeModal();});
  // Native date pickers can remain open after a day is chosen on Safari/iPad.
  // Blurring a completed date input commits the selection and closes the popover.
  root.querySelectorAll('input[type="date"]').forEach(input=>{
    input.addEventListener('change',()=>{
      if(input.value)requestAnimationFrame(()=>input.blur());
    });
  });
  setTimeout(()=>root.querySelector('input,textarea,select')?.focus(),0);
}
...
async function init(){
  ...
  window.addEventListener('pagehide',()=>{
    flushMainLocalWrites().catch(()=>{});
    cloudApi()?.flushNow?.().catch(()=>{});
  });
