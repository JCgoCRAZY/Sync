...
// -----------------------------------------------------------------------------
// GitHub Pages deployment version indicator
// Displays the latest successful GitHub Pages deployment in Toronto time as
// YYYY.MM.DD-HH:MM. This is fetched once when the app opens and cached locally
// so the last confirmed deployment remains visible while offline.
// -----------------------------------------------------------------------------
(() => {
  const VERSION_REPOSITORY = 'JCgoCRAZY/Sync';
  const VERSION_CACHE_KEY = 'ushub:last-confirmed-pages-deployment';
  const VERSION_CHECKED_AT_KEY = 'ushub:last-pages-version-check';
  let deploymentVersion = localStorage.getItem(VERSION_CACHE_KEY) || '';

  function formatTorontoDeploymentTime(isoValue) {
  ...
  function versionText() {
    return deploymentVersion ? `Version ${deploymentVersion}` : 'Version checking…';
  }

  function updateVersionDisplays() {
    document.querySelectorAll('[data-deployment-version]').forEach(el => {
      el.textContent = versionText();
      el.title = deploymentVersion
        ? `Latest successful GitHub Pages deployment: ${deploymentVersion} (Toronto time)`
        : 'Checking the latest successful GitHub Pages deployment…';
    });
  }

  function installTopVersionDisplay() {
    const header = document.querySelector('.top-header');
    if (!header || header.querySelector('.deployment-version-top')) return;

    header.style.position = 'relative';

    const el = document.createElement('div');
    el.className = 'deployment-version-top';
    el.dataset.deploymentVersion = '';
    el.textContent = versionText();
    header.appendChild(el);

    if (!document.getElementById('deployment-version-styles')) {
      const style = document.createElement('style');
      style.id = 'deployment-version-styles';
      style.textContent = `
        .deployment-version-top {
          position: absolute;
          left: 50%;
          top: 50%;
  ...
  async function fetchLatestPagesDeploymentVersion() {
    try {
      const response = await fetch(
        `https://api.github.com/repos/${VERSION_REPOSITORY}/actions/runs?branch=main&status=success&per_page=30`,
        {
          cache: 'no-store',
      ...
      // Prefer the GitHub Pages workflow specifically rather than an unrelated CI run.
      const pagesRun = runs.find(run => {
        const description = `${run?.name || ''} ${run?.display_title || ''} ${run?.path || ''}`;
        return /pages/i.test(description) && /(build|deploy)/i.test(description);
      });

      if (!pagesRun) throw new Error('No successful GitHub Pages deployment was found.');

      const deployedAt = pagesRun.updated_at || pagesRun.run_started_at || pagesRun.created_at;
      const formatted = formatTorontoDeploymentTime(deployedAt);
      if (!formatted) throw new Error('GitHub returned an invalid deployment time.');

      deploymentVersion = formatted;
      localStorage.setItem(VERSION_CACHE_KEY, deploymentVersion);
      localStorage.setItem(VERSION_CHECKED_AT_KEY, new Date().toISOString());
      updateVersionDisplays();
    } catch (err) {
      console.warn('Could not refresh GitHub Pages deployment version:', err);
      updateVersionDisplays();
    }
  }

  // Add the same version to Settings without altering the existing Settings logic.
  const previousRenderSettings = renderSettings;
  renderSettings = function renderSettingsWithDeploymentVersion() {
    ...
    section.innerHTML = `
      <div class="card-header">
        ...
        <div class="settings-version-value" data-deployment-version></div>
      </div>
    `;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    installTopVersionDisplay();
    updateVersionDisplays();
    fetchLatestPagesDeploymentVersion();
  });

  // If this script happens to load after DOMContentLoaded, initialize immediately.
