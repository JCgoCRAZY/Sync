(() => {
  ...
  async function getMeta(userId = currentUserId) {
    ...
    return {
      ...
      // v3 hashes intentionally exclude device-local editor_layout. Do not compare
  ...
  async function replaceCloudFromLocal(remoteState = null) {
    ...
    for (const [id, doc] of localDocs) {
      ...
      const row = { user_id: currentUserId, document_id: String(id), document: cloudDoc, revision, updated_at: nowIso(), updated_by_device: deviceId(), deleted_at: null };
  ...
  async function pushDocument(id, queue, meta) {
    ...
    let data = null, error = null;
    if (base === 0) {
      ({ data, error } = await client.from(CONFIG.documentTable).insert({ user_id: currentUserId, document_id: id, document: cloudDoc, revision: 1, updated_at: updated, updated_by_device: deviceId(), deleted_at: null }).select().maybeSingle());
    } else {
      ({ data, error } = await client.from(CONFIG.documentTable).update({ document: cloudDoc, revision: base + 1, updated_at: updated, updated_by_device: deviceId(), deleted_at: null })
    ...
    if (!data) {
      ...
      if (remoteResult.data && !remoteResult.data.deleted_at && sameValue(cloudDoc, documentForCloud(remoteResult.data.document || {}))) {
        ...
        queue.dirtyDocs = quickHash(documentForCloud(latestDoc || {})) === pushedHash
    ...
    flushPromise = (async () => {
      ...
      try {
        ...
        if (remaining) {
          emitStatus({
            phase: 'pending',
            message: `${remaining} newer local change${remaining === 1 ? '' : 's'} still waiting to sync.`,
